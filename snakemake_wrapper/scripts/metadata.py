...
for url in ftp_list:
    try:
        ...
        gse_df.drop(columns=gse_df.columns.difference(columns_to_select), inplace=True)
        ...
        gsm_ids = []
        for string in final_srp_df["experiment_title"]:
            gsm_id = string.split(":", 1)[0]
            gsm_ids.append(gsm_id)
            
        final_srp_df['geo_accession'] = gsm_ids
